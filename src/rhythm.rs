...
/// Local DP identical in spirit to Measure::best_fill_for_gap, but self-contained for the rhythm module.
fn best_fill_for_gap(gap_ticks: i32) -> Option<Vec<Duration>> {
    ...
    let mut coins: Vec<(i32, Duration, i32)> = Duration::DURATIONS
        ...
        .map(|&d| {
            ...
            let weight = Duration::denominator_of(d); // smaller denominators preferred
            (ticks, d, weight)
        })
        .collect();
